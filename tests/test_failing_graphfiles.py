...
class QuitAppTestCase(unittest.TestCase):
    ...
    def testInitWithGraphfiles(self):
        ...
        # example1.nq should contain the new quad but doesn't
        self.assertNotEqual(file_example1_before, file_example1_after)

        # the quad was moved to unassigned.nq and named graph changed to <http://example.org/2>
        self.assertFalse(os.path.isfile(join(self.tmpdir.name, 'unassigned.nq')))

        with open(join(self.tmpdir.name, 'unassigned.nq'), 'r') as f:
            unassigned_content = f.read()

        content = '<newSub> <newPred> <newObj> <http://example.org/2/> .'
